class Character{
  ...
  Character(
      this.id, this.name, this.species, this.type, this.image, this.status, this.gender);

  factory Character.fromJSON(dynamic data) {
    return Character(data["id"], data["name"], data["species"], data["type"], data["image"], data["status"], data["gender"]);
  }
}
...
class Location{
  ...
  Location(this.id, this.name, this.type, this.dimension) : residents = <Character>[];
...
class Episode{
  ...
  Episode(this.id, this.name, this.air_date, this.episode) : characters = <Character>[];
